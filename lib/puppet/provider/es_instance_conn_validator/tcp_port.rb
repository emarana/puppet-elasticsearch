...
# which validates the elasticsearch-legacy instance connection by attempting an https connection.
...
Puppet::Type.type(:es_instance_conn_validator).provide(:tcp_port) do
  desc "A provider for the resource type `es_instance_conn_validator`,
        ...
        connection to the elasticsearch-legacy instance." 
  ...
  def exists?
    ...
    while success == false && ((Time.now - start_time) < timeout)
      # It can take several seconds for the elasticsearch-legacy instance to start up;
      ...
      Puppet.debug("Failed to connect to the elasticsearch-legacy instance; sleeping 2 seconds before retry")
      sleep 2
      success = validator.attempt_connection
    end
